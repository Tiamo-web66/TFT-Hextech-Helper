...
const SettingsPage = () => {
    ...
    const handleWindowOcclusionBehaviorChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        setWindowOcclusionBehavior(value);
        await settingsStore.setWindowOcclusionBehavior(value);
        toast.success(value === WindowOcclusionBehavior.AUTO_FOCUS
            ? '已切换为自动聚焦模式'
            : '已切换为暂停提醒模式'
        );
    };
            ...
            <SettingsCard>
                <SettingItem>
                    <SettingInfo>
                        <SettingText>
                            <h3>窗口遮挡处理</h3>
                            <p>当游戏窗口被遮挡或切换到其他窗口时的处理方式。</p>
                        </SettingText>
                    </SettingInfo>
                    <SelectWrapper
                        value={windowOcclusionBehavior}
                        onChange={handleWindowOcclusionBehaviorChange}
                    >
                        <option value={WindowOcclusionBehavior.AUTO_FOCUS}>自动聚焦</option>
                        <option value={WindowOcclusionBehavior.PAUSE_AND_WARN}>暂停并提醒</option>
                    </SelectWrapper>
                </SettingItem>
            </SettingsCard>
