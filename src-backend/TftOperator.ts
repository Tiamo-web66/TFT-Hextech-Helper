...
<<<<<<< HEAD
    ...
    public async initAsync(): Promise<boolean> {
        try {
            ...
            logger.info(
                `[TftOperator] 游戏基准点(动态检测): (${this.gameWindowRegion.x}, ${this.gameWindowRegion.y}) ` +
                `窗口: "${findResult.title}"`
            );
     ...
     * @returns 窗口状态结果
     */
    public async updateWindowPosition(): Promise<WindowStatusResult> {
        const status = await windowFinder.checkWindowStatus(this.gameWindowRegion ?? undefined);
     ...
     */
    public async ensureWindowReady(): Promise<boolean> {
